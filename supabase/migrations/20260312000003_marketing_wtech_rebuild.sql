...
CREATE TABLE IF NOT EXISTS "SITE_MarketingLists" (
    ...
    name TEXT NOT NULL,
    description TEXT,
    owner_id UUID REFERENCES auth.users(id), -- Link to custom auth table
...
CREATE TABLE IF NOT EXISTS "SITE_MarketingListMembers" (
    ...
    lead_id UUID REFERENCES leads(id) ON DELETE SET NULL, -- specific lead link
...
ALTER TABLE public.email_templates ADD COLUMN IF NOT EXISTS variables TEXT[]; -- Array of expected vars e.g. ['name', 'course']
...
ALTER TABLE IF EXISTS public.email_templates ADD COLUMN IF NOT EXISTS "imageUrl" TEXT;
ALTER TABLE IF EXISTS public.email_templates ADD COLUMN IF NOT EXISTS "content2" TEXT;
